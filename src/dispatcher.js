...
export class AVSDispatcher {
  constructor(args) {
    ...
    initregionscb()
    setInterval(initregionscb, 1 * 24 * 60 * 60 * 1000) // update in database at least once a day
  }
  ...
  async initRegions() {
    const regions = process.env.REGIONS.split(' ')
      ...
      .map((el) => ({
        ...
        expireAt: Date.now() + 2 * 24 * 60 * 60 * 1000 // means a restart every day
      }))
  ...
  installHandlers(path, app) {
    ...
    app.put(path + '/router', async (req, res) => {
      ...
      toinsert.expireAt = Date.now() / 1000 + 60 * 1000
      ...
      try {
        ...
        if (upres.modifiedCount + upres.upsertedCount === 0)
          return res.status(500).send('db error')
        if (upres.upsertedCount > 1) {
          const hashSalt = (
